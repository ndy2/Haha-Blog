...
> * 쉬운코드 - [(2 부) DB 정규화 (normalization) 2 부입니다!!]
...
* 중복 데이터가 생겼습니다.
* ratio 의 개념이 이상해 졌습니다. (한 직원에 대해서 합이 2 가되었습니다.)
* PK 에도 변경이 필요해 졌습니다.
...
card_id 를 분리해서 테이블에 저장함으로써 기존의 `{account_id}`, `{bank_name, account_num}` 은 candidate key 로 동작할 수 없습니다. 모두 `card_id` 까지 포함되어야 tuple 을 유일하게 결정하는 candidate key 가 될 수 있습니다.
