...
1. 모든 앞 필터를 다 통과하고 `AuthorizationFilter` 에 도착한다.
2. 그 과정에서 `SecurityContextHolder` 에는 `AnonymousAuthenticationFilter` 에 의해 `AnonymousAuthenticationToken` 이 저장된다.
3. 권한 부여 대상이 아닌 경우를 판단 하기 위해 전처리를 살짝 해 준다.

![[images/authorizationfilter-dofilter.png]]

1. 가지고 있는 authorizationManager 의 check 을 통해 AccessDecision 획득하고 이를 통해 ACCESS_ABSTAIN 혹은 ACCESS_DENIED 라면 직접 예외를 터트린다.
