...
---
자바의 NIO (New IO) 는 자바 4에 추가된 IO API로 기존 자바 IO 와는 다른 방식으로 동작합니다. NIO 는 Non-blocking IO의 줄임말 처럼 생각될 수 있지만 NIO는 실제로 Non-blocking IO 와 Blocking IO 방식 모두 지원합니다. 기존의 Java IO 를 BIO 라고 부르기도 합니다.

자바 7부터는 NIO2 가 지원되었습니다.
