...
!!! note "인터넷 제어 메시지 프로토콜 (ICMP)은 무엇인가요?"
    ...
    :warning: - ICMP는 통신 환경의 문제점에 관한 피드백을 제공하지만 IP가 신뢰 가능하도록 만들지는 않습니다. 즉, ICMP는 IP 패킷이 확실하게 전달된다거나 IP 패킷이 전달되지 않을 때 또는 올바르지 않게 전달될 때 ICMP 메시지가 소스 호스트로 리턴된다고 보장하지 않습니다.
...
!!! note "LAN 과 WAN은 어떻게 구분하나요?"
    ...
    LAN 
    -  물리적인 영역 (OSI - 1계층, 2계층, TCP/IP - Access 계층)에서 설명되는 영역
    - 즉, MAC Address 가 중요한 영역
...
!!! quote "더 자세히 알아볼 키워드 들"
    ...
    physical infrastructure - 라우터, DNS  
