...
!!! quote "CAP theorem on [wikipedia](https://en.wikipedia.org/wiki/CAP_theorem)"
    ...
    In theoretical computer science, the **CAP theorem** states that any distributed data store can provide only two of the following three quarantees <br>
    
    * Consistency - Every read receives the most recent write or an error
    * Availability - Every request receives a (non-error) response, without the guarantee that it contains the most recent write.
    * Partition tolerance - The system continues to operate despite an arbitrary number of messages being dropped (or delayed) by the network between nodes.

![the-cap-theorem-diagram.png](images/the-cap-theorem-diagram.png)
