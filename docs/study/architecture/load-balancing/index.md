...
![load-balancing.png](images/load-balancing.png)
...
!!! note "Hash & IP Hash"
    ...
    - 단점
        - 균등한 분배가 보장되지 않는다
        - 서버가 추가, 제거되는 경우 Rehashing 이 발생한다
            - 알고리즘 적으로 rehashing 의 발생 빈도를 낮출 수는 있지만 완벽하지는 않다.
...
!!! tip
    ...
    서버의 성능이 같지 않다면 서버별 weight 를 줄 수 있다.
